...
def generate_available_dates_json():
    date_set = set()
    pattern = re.compile(r"JEPI_.*_(\d{4}-\d{2}-\d{2})\.json")
...
def main():
    ...
    excel_filename = os.path.join(DATA_FOLDER, f'JEPI_{date_str}.xlsx')
    ...
    for bucket_name in ["Options - Index", "Cash", "Stocks"]:
        ...
        if bucket_name == "Options - Index":
            ...
            opening_price = CONFIG["SPX"]  # hardcoded underlying price
    ...
    for bucket_name in ["Options - Index", "Cash", "Stocks"]:
        dated_file = os.path.join(DATA_FOLDER, f'JEPI_{bucket_name.replace(" ", "_")}_{date_str}.json')
        latest_file = os.path.join(DATA_FOLDER, f'JEPI_{bucket_name.replace(" ", "_")}_latest.json')
